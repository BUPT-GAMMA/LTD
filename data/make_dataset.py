...
def get_dataset(name, data_path, standardize, train_examples_per_class=None, val_examples_per_class=None):
    ...
    assert is_binary_bag_of_words(
        node_features), f"Non-binary node_features entry!"
...
def get_train_val_test_split(random_state,
    ...
    print(train_examples_per_class, val_examples_per_class,
          test_examples_per_class, train_size, val_size, test_size)
    train_base = []
    val_base = []
    forbidden_indices = np.concatenate((train_base, val_base))
    if train_examples_per_class is not None:
        train_indices = sample_per_class(
            random_state, labels, 20, forbidden_indices=forbidden_indices)
    else:
        # select train examples with no respect to class distribution
        train_indices = random_state.choice(
            remaining_indices, train_size, replace=False, forbidden_indices=train_base)
    
    train_indices = np.int64(np.concatenate((train_indices, train_base)))
    forbidden_indices = np.concatenate((train_indices, val_base))
    ...
    val_indices = np.int64(np.concatenate((val_indices, val_base)))
    forbidden_indices = np.concatenate((train_indices, val_indices))
    if my_val_per_class is not None:
        my_val_indices = sample_per_class(
            random_state, labels, my_val_per_class, forbidden_indices=forbidden_indices)
    ...
    if test_size is None and test_examples_per_class is None:
        ...
        assert len(np.concatenate(
            (train_indices, val_indices, test_indices, my_val_indices))) == num_samples
    ...
    return train_indices, val_indices, test_indices, my_val_indices
...
def get_split_feed_dicts(train_indices, val_indices, test_indices):
    dataset_indices_placeholder = tf.placeholder(
        tf.int32, shape=[None], name='dataset_indices_placeholder')

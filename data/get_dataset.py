...
def generate_data_path(dataset, dataset_source):
    if dataset_source == 'planetoid':
        ...
        raise ValueError(
            f'The "dataset_source" must be set to "planetoid" or "npz"')
...
def load_dataset_and_split(labelrate, dataset):
    ...
    _config['data_path'] = generate_data_path(
        dataset, _config['dataset_source'])
    if _config['dataset_source'] == 'planetoid':
        ...
        idx_train, idx_val, idx_test, idx_my_val = get_train_val_test_split(random_state, labels,
                                                                **_config['train_config']['split'])
        return adj, features, labels, idx_train, idx_val, idx_test, idx_my_val


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='load data', formatter_class=argparse.RawTextHelpFormatter)
    ...
    adj, features, labels, idx_train, idx_val, idx_test, idx_my_val = load_dataset_and_split(
        args.config_file)

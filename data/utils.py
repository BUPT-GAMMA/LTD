...
def load_tensor_data(model_name, dataset, labelrate, device):
    if dataset in ['composite', 'composite2', 'composite3']:
        adj, features, labels_one_hot, idx_train, idx_val, idx_test = load_composite_data(
            dataset)
    else:
        # config_file = os.path.abspath('data/dataset.conf.yaml')
        adj, features, labels_one_hot, idx_train, idx_val, idx_test, idx_my_val = load_dataset_and_split(
            labelrate, dataset)
    ...
    return adj, adj_sp, features, labels, labels_one_hot, idx_train, idx_val, idx_test, idx_my_val
